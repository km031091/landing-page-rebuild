...
serve(async (req) => {
  ...
  try {
    ...
    await supabaseClient.from("appointments").delete().eq("professional_id", userId);
    logStep("Deleted appointments");
    ...
    if (deleteError) throw new Error(`Error deleting auth user: ${deleteError.message}`);
